...
def predict(
    ...
    """预测入口。n_steps=60 为标准 LSTM，n_steps=15 为轻量模型（小数据）"""
    all_predictions_lstm = {}
    prediction_metrics = {}

    print(f"\nProcessing {ticker_name} (n_steps={n_steps})")
    data = stock_data
    X, y = stock_features
    
    predict_result, test_indices, predictions, actual_percentages = train_and_predict_lstm(
        ticker_name, data, X, y, save_dir, n_steps=n_steps, num_epochs=epochs, batch_size=batch_size, learning_rate=learning_rate
    )
